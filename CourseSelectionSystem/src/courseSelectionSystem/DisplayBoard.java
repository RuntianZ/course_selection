...
class DisplayBoard extends JPanel {
	...
	public DisplayBoard(Displayable data) {
		int i = 0;
		data.displayPrepare();
		setLayout(new VFlowLayout());
		Iterator<DisplayContent> it = data.iterator();
		while(it.hasNext()) {
			DisplayContent c = it.next();
			JPanel p = new JPanel();
			p.setLayout(new BorderLayout());
			if(i % 2 == 0)
				p.setBackground(Color.WHITE);
			else
				p.setBackground(new Color(200,200,245));
			JTextArea text = new JTextArea(c.getCategory());
			text.setOpaque(false);
			text.setFont(LanguageManager.getFont(Font.BOLD, 20));
			p.add(text, BorderLayout.WEST);
			{
				JPanel panel = new JPanel();
				panel.setOpaque(false);
				panel.setLayout(new FlowLayout());
				panel.add(c.getContent());
